...
@app.route("/api/sectioned-images/<job_id>", methods=["GET"])
def get_sectioned_images_list(job_id):
    """Get a list of all sectioned images for a job"""
    try:
        # Construct the path to the sections directory
        sections_directory = os.path.join(images_directory, f"{job_id}/{job_id}_sections")
        ...
        if not os.path.exists(sections_directory):
            return jsonify({
                "success": False,
                "error": {"code": "ERR_DIRECTORY_NOT_FOUND", "message": "Sections directory not found"}
            }), 404
        
        # Get all files in the directory
        section_files = [f for f in os.listdir(sections_directory) if f.endswith(".png")]
        
        # Format the response data
        sections = [{
            "filename": f,
            "path": f"/api/sectioned-image/{job_id}/{os.path.splitext(f)[0]}",
            "index": i
        } for i, f in enumerate(sorted(section_files))]
        ...
        return jsonify({
            "success": False,
            "error": {"code": "ERR_LISTING_SECTIONS", "message": str(e)}
        }), 500

@app.route("/api/sectioned-image/<job_id>/<section_name>", methods=["GET"])
def get_sectioned_image(job_id, section_name):
    """Serve a specific sectioned image"""
    try:
        # Construct the path to the sectioned image
        image_path = os.path.join(images_directory, f"{job_id}/{job_id}_sections/{section_name}.png")
        
        # Check if the file exists
        if not os.path.exists(image_path):
            return jsonify({
                "success": False,
                "error": {"code": "ERR_FILE_NOT_FOUND", "message": "Image not found"}
            }), 404
        ...
        return jsonify({
            "success": False,
            "error": {"code": "ERR_SERVING_IMAGE", "message": str(e)}
        }), 500
